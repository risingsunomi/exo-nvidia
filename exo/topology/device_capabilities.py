...
def mac_device_capabilities() -> DeviceCapabilities:
  ...
  model = subprocess.check_output(["system_profiler", "SPHardwareDataType"]).decode(
    "utf-8"
  )
  model_line = next(
...
def get_tinygrad_device() -> tuple[str, str, any]:
  import psutil
  ...
  # default gpu_mem
  gpu_memory_info = psutil.virtual_memory().total // 2**20
  ...
  if Device.DEFAULT == "CUDA" or Device.DEFAULT == "NV" or Device.DEFAULT == "GPU":
    ...
    return (Device.DEFAULT, gpu_name, gpu_memory_info)
  elif Device.DEFAULT == "AMD":
    return ("AMD", "Unknown AMD", gpu_memory_info)
  else:
    return (Device.DEFAULT, "Unknown", gpu_memory_info)


def linux_device_capabilities() -> DeviceCapabilities:
  device, gpu_name, gpu_mem = get_tinygrad_device()

  if DEBUG >= 2:
    print(f"tinygrad {device=}")

  if device == "CUDA" or device == "NV" or device == "GPU":
    return DeviceCapabilities(
      model=f"Linux Box ({gpu_name})",
      chip=gpu_name,
      memory=gpu_mem.total // 2**20,
    ...
    return DeviceCapabilities(
      model=f"Linux Box (Device: {device})",
      chip=f"Unknown Chip (Device: {device})",
      memory=gpu_mem,
      flops=DeviceFlops(fp32=0, fp16=0, int8=0),
    )
...
def windows_device_capabilities() -> DeviceCapabilities:
    ...
    if device == "CUDA" or device == "NV" or device == "GPU":
      ...
      return DeviceCapabilities(
        model=f"Windows Box (Device: {device})",
        chip=f"Unknown Chip (Device: {device})",
        memory=gpu_mem,
        flops=DeviceFlops(fp32=0, fp16=0, int8=0),
      )
